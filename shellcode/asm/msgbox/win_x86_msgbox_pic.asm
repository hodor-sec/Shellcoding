    ...
    resolve_symbols_kernel32:
        ...
        jmp load_user_32                ; Jump over call_getlasterror
    ...
    load_user_32:
        ...
        mov ax, 0x6c6c                   ; "ll"
        push ax
        push 0x642e3233                  ; "32.dll"
        push 0x72657375                  ; resu
        push esp                         ; PTR to string
        call dword ptr [ebp+0x18]        ; Call LoadLibraryA
        test eax, eax
        jz call_getlasterror             ; If resulting error is zero, call getlasterror        

    resolve_symbols_user32:
        mov ebx, eax                    ; Copy address of loaded lib into EBX
        push 0xbc4da2a8                 ; MessageBoxA hash
        call dword ptr [ebp+0x04]       ; Call find function
        mov [ebp+0x1c], eax             ; Save MessageBoxA       
    
    call_messageboxa:
        xor eax, eax                    ; EAX = NULL
        push ax                         ; String terminate
        mov ax, 0x4f4c                  ; OL
        push ax                             
        push 0x4c45484f                 ; LEHO
        push 0x4c4c4548                 ; LLEH
        push esp                        ; &HELLO
        pop ebx                         ; Save ptr in EBX
        xor eax, eax                    ; EAX = NULL
        push eax                        ; Push uType
        push ebx                        ; Push lpCaption
        push ebx                        ; Push lpText
        push eax                        ; Push hWnd
        call dword ptr [ebp+0x1c]       ; Call MessageBoxA        
